...
public:
    ...
    [[nodiscard]] std::optional<uint8_t> block_at(const nnm::Vector3i block_pos) const
    {
        ...
        if (chunk_pos.y < -10 || chunk_pos.y >= 10) {
            return {};
        }
        if (!m_chunk_columns.contains({ chunk_pos.x, chunk_pos.z })) {
            return {};
        }
        return m_chunk_columns.at({ chunk_pos.x, chunk_pos.z }).get_block(block_pos);
    }

    void set_lighting(const nnm::Vector3i pos, const uint8_t val)
    {
        nnm::Vector3i chunk_pos = chunk_pos_from_block_pos(pos);
        VV_DEB_ASSERT(m_chunk_columns.contains({ chunk_pos.x, chunk_pos.z }), "[WorldData] Invalid chunk");
        m_chunk_columns.at({ chunk_pos.x, chunk_pos.z }).set_lighting(pos, val);
    }
    ...
    [[nodiscard]] std::optional<uint8_t> lighting_at(const nnm::Vector3i block_pos) const
    {
        ...
        return m_chunk_columns.at({ chunk_pos.x, chunk_pos.z }).lighting_at(block_pos);
    }
    ...
    void set_block(const nnm::Vector3i block_pos, const uint8_t type)
    {
        nnm::Vector3i chunk_pos = chunk_pos_from_block_pos(block_pos);
        VV_DEB_ASSERT(m_chunk_columns.contains({ chunk_pos.x, chunk_pos.z }), "[WorldData] Invalid chunk");
        m_chunk_columns.at({ chunk_pos.x, chunk_pos.z }).set_block(block_pos, type);
        queue_save_chunk({ chunk_pos.x, chunk_pos.z });
    }

    void set_block_local(nnm::Vector3i chunk_pos, const nnm::Vector3i block_pos, const uint8_t type)
    {
        VV_DEB_ASSERT(m_chunk_columns.contains({ chunk_pos.x, chunk_pos.z }), "[WorldData] Invalid chunk");
        m_chunk_columns.at({ chunk_pos.x, chunk_pos.z }).set_block(block_local_to_world(chunk_pos, block_pos), type);
        queue_save_chunk({ chunk_pos.x, chunk_pos.z });
    }
    ...
    {
        VV_DEB_ASSERT(m_chunk_columns.contains({ chunk_pos.x, chunk_pos.z }), "[WorldData] Invalid chunk");
        return m_chunk_columns.at({ chunk_pos.x, chunk_pos.z }).chunk_data_at(chunk_pos);
    }
    ...
    {
        return chunk_pos.y >= -10 && chunk_pos.y < 10 && m_chunk_columns.contains({ chunk_pos.x, chunk_pos.z });
    }
