...
public:
    ...
    [[nodiscard]] uint8_t get_block(const nnm::Vector3i block_pos) const
    {
        ...
        VV_DEB_ASSERT(chunk_pos.y >= -10 && chunk_pos.y < 10, "[ChunkColumn] Invalid block position");
        return m_chunks[chunk_pos.y + 10].get_block(block_world_to_local(block_pos));
    }
    ...
    void set_lighting(const nnm::Vector3i block_pos, const uint8_t val)
    {
        const nnm::Vector3i chunk_pos = chunk_pos_from_block_pos(block_pos);
        VV_DEB_ASSERT(chunk_pos.y >= -10 && chunk_pos.y < 10, "[ChunkColumn] Invalid block position");
        m_chunks[chunk_pos.y + 10].set_lighting(block_world_to_local(block_pos), val);
    }
    ...
    [[nodiscard]] uint8_t lighting_at(const nnm::Vector3i block_pos) const
    {
        const nnm::Vector3i chunk_pos = chunk_pos_from_block_pos(block_pos);
        VV_DEB_ASSERT(chunk_pos.y >= -10 && chunk_pos.y < 10, "[ChunkColumn] Invalid block position");
        return m_chunks[chunk_pos.y + 10].lighting_at(block_world_to_local(block_pos));
    }
    ...
    void set_block(const nnm::Vector3i block_pos, const uint8_t type)
    {
        const nnm::Vector3i chunk_pos = chunk_pos_from_block_pos(block_pos);
        VV_DEB_ASSERT(chunk_pos.y >= -10 && chunk_pos.y < 10, "[ChunkColumn] Invalid block position");
        m_chunks[chunk_pos.y + 10].set_block(block_world_to_local(block_pos), type);
    }

    [[nodiscard]] const ChunkData& chunk_data_at(const nnm::Vector3i chunk_pos) const
    {
        VV_DEB_ASSERT(
            chunk_pos.x == m_col_pos.x && chunk_pos.y >= -10 && chunk_pos.y < 10 && chunk_pos.z == m_col_pos.y,
            "[ChunkColumn] Invalid chunk position");
        return m_chunks[chunk_pos.y + 10];
    }
    ...
    {
        archive(m_col_pos, m_chunks, m_gen_level);
    }
