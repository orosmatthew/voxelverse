    ...
    UniformLocation UniformStructLayout::calculate_location(const UniformStructLayout::Variable& variable)
    {
        if (!variable.is_array && !variable.is_struct) {
            ...
            case UniformType::e_float:
                m_location_offset = glm::ceil(m_location_offset / (float)sizeof(float)) * sizeof(float);
                location = UniformLocation(m_location_offset);
                m_location_offset += sizeof(float);
                m_size_bytes += sizeof(float);
                return location;
            case UniformType::e_vec2:
                m_location_offset = glm::ceil(m_location_offset / (float)sizeof(glm::vec2)) * sizeof(glm::vec2);
                location = UniformLocation(m_location_offset);
                m_location_offset += sizeof(glm::vec2);
                m_size_bytes += sizeof(glm::vec2);
                return location;
            case UniformType::e_vec3:
                // NOTE: this is correct, base alignment of vec3 is same as vec4
                m_location_offset = glm::ceil(m_location_offset / (float)sizeof(glm::vec4)) * sizeof(glm::vec4);
                location = UniformLocation(m_location_offset);
                m_location_offset += sizeof(glm::vec4);
            ...
            case UniformType::e_mat2:
                m_location_offset
                    = glm::ceil(m_location_offset / (float)sizeof(glm::mat2::col_type)) * sizeof(glm::mat2::col_type);
                location = UniformLocation(m_location_offset);
                m_location_offset += sizeof(glm::mat2);
                m_size_bytes += sizeof(glm::mat2);
                return location;
            case UniformType::e_mat3:
                m_location_offset
                    = glm::ceil(m_location_offset / (float)sizeof(glm::mat3::col_type)) * sizeof(glm::mat3::col_type);
                location = UniformLocation(m_location_offset);
                m_location_offset += sizeof(glm::mat3);
                m_size_bytes += sizeof(glm::mat3);
                return location;
            case UniformType::e_mat4:
                m_location_offset
                    = glm::ceil(m_location_offset / (float)sizeof(glm::mat4::col_type)) * sizeof(glm::mat4::col_type);
                location = UniformLocation(m_location_offset);
                m_location_offset += sizeof(glm::mat4);
