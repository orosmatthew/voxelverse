...
UIPipeline::UIPipeline(std::shared_ptr<mve::Renderer> renderer)
    : m_renderer(renderer)
    , m_vertex_shader(mve::Shader("../res/bin/shader/ui.vert.spv"))
    , m_fragment_shader(mve::Shader("../res/bin/shader/ui.frag.spv"))
    , m_graphics_pipeline(
          renderer->create_graphics_pipeline(m_vertex_shader, m_fragment_shader, vertex_layout(), false))
...
void UIPipeline::update_framebuffer_texture(const mve::Texture& texture, mve::Vector2i size)
{
    ...
    if (!m_world.has_value()) {
        World world { .vertex_buffer = renderer_ref->create_vertex_buffer(world_data),
                      .index_buffer = renderer_ref->create_index_buffer({ 0, 3, 2, 0, 2, 1 }),
                      .descriptor_set = m_graphics_pipeline.create_descriptor_set(m_vertex_shader.descriptor_set(1)),
                      .uniform_buffer
                      = renderer_ref->create_uniform_buffer(m_vertex_shader.descriptor_set(1).binding(0)),
        ...
        m_world->vertex_buffer = renderer_ref->create_vertex_buffer(world_data);
...
UIUniformData UIPipeline::create_uniform_data()
{
    UIUniformData data
        = { .descriptor_set = m_graphics_pipeline.create_descriptor_set(m_vertex_shader.descriptor_set(1)),
            .buffer = lock_renderer()->create_uniform_buffer(m_vertex_shader.descriptor_set(1).binding(0)) };
...
{
    lock_renderer()->bind_graphics_pipeline(m_graphics_pipeline);
}
    ...
    const mve::IndexBuffer& index_buffer) const
{
    ...
    renderer_ref->bind_descriptor_sets(m_global_descriptor_set, descriptor_set);
    renderer_ref->bind_vertex_buffer(vertex_buffer);
    renderer_ref->draw_index_buffer(index_buffer);
}
void UIPipeline::draw_world() const
{
    ...
    renderer_ref->bind_graphics_pipeline(m_graphics_pipeline);
    if (m_world.has_value()) {
        renderer_ref->bind_descriptor_sets(m_global_descriptor_set, m_world->descriptor_set);
        renderer_ref->bind_vertex_buffer(m_world->vertex_buffer);
        renderer_ref->draw_index_buffer(m_world->index_buffer);
    }
}
