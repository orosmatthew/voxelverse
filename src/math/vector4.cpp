...
Vector4 direction_to(Vector4 from, Vector4 to)
{
    return mve::normalize(to - from);
}
float distance_squared_to(Vector4 from, Vector4 to)
{
...
float distance_to(Vector4 from, Vector4 to)
{
    float diff_x = to.x - from.x;
    float diff_y = to.y - from.y;
    float diff_z = to.z - from.z;
...
float length_squared(Vector4 vector)
{
    return squared(vector.x) + squared(vector.y) + squared(vector.z) + squared(vector.w);
}
Vector4 linear_interpolate(Vector4 from, Vector4 to, float weight)
{
    return Vector4(
...
Vector4Axis min_axis(Vector4 vector)
{
...
Vector4Axis max_axis(Vector4 vector)
{
...
Vector4 inverse(Vector4 vector)
{
    return Vector4(1.0f / vector.x, 1.0f / vector.y, 1.0f / vector.z, 1.0f / vector.w);
}
Vector4 clamp_length(Vector4 vector, float min, float max)
{
...
Vector4 Vector4::direction_to(Vector4 to) const
{
    return mve::direction_to(*this, to);
}
float Vector4::distance_squared_to(Vector4 to) const
{
    return mve::distance_squared_to(*this, to);
}
float Vector4::distance_to(Vector4 to) const
{
...
Vector4 Vector4::linear_interpolate(Vector4 to, float weight) const
{
    return mve::linear_interpolate(*this, to, weight);
}
Vector4Axis Vector4::min_axis() const
{
    return mve::min_axis(*this);
}
Vector4Axis Vector4::max_axis() const
{
    return mve::max_axis(*this);
}
bool Vector4::is_equal_approx(Vector4 vector) const
{
...
float Vector4::dot(Vector4 vector) const
{
...
Vector4 Vector4::clamp_length(float min, float max) const
{
...
Vector4& Vector4::operator*=(int val)
{
    x *= val;
    y *= val;
    z *= val;
...
float& Vector4::operator[](Vector4Axis axis)
{
    switch (axis) {
    case Vector4Axis::x:
        return x;
