...
template <typename Number>
bool is_zero_approx(const Number value, Number epsilon)
{
    return mve::abs(value) < epsilon;
}

template <typename Number>
bool is_equal_approx(const Number a, const Number b, Number epsilon)
{
    if (a == b) {
        return true;
    }
...
public:
    ...
    {
        return { mve::ceil(x), mve::ceil(y) };
    }

    [[nodiscard]] Vector2 clamp(const Vector2& min, const Vector2& max) const
    {
        return { mve::clamp(x, min.x, max.x), mve::clamp(y, min.y, max.y) };
    }
    ...
    [[nodiscard]] Number distance_sqrd_to(const Vector2& to) const
    {
        ...
        return mve::sqrd(diff_x) + mve::sqrd(diff_y);
    }

    [[nodiscard]] Number distance_to(const Vector2& to) const
    {
        return mve::sqrt(this->distance_sqrd_to(to));
    }

    [[nodiscard]] Vector2 floor() const
    {
        return { mve::floor(x), mve::floor(y) };
    }

    [[nodiscard]] Number length_sqrd() const
    {
        return mve::sqrd(x) + mve::sqrd(y);
    }

    [[nodiscard]] Number length() const
    {
        return mve::sqrt(length_sqrd());
    }
    ...
    [[nodiscard]] bool is_equal_approx(const Vector2& other, const Number epsilon) const
    {
        return mve::is_equal_approx(x, other.x, epsilon) && mve::is_equal_approx(y, other.y, epsilon);
    }

    [[nodiscard]] bool is_zero_approx(Number epsilon) const
    {
        return mve::is_zero_approx(x, epsilon) && mve::is_zero_approx(y, epsilon);
    }

    [[nodiscard]] Vector2 move_toward(const Vector2& to, const Number amount) const
    {
        if (this->distance_to(to) >= amount) {
            return to;
        }
        Vector2 result = *this;
        result += this->direction_to(to) * amount;
        return result;
    }
    ...
    {
        Vector2 result;
        result.x = x * mve::cos(angle) - y * mve::sin(angle);
        result.y = x * mve::sin(angle) + y * mve::cos(angle);
        return result;
    }
    ...
    {
        auto result = *this;
        if (const Number length_sqr = result.length_sqrd(); length_sqr > 0.0) {
            result = result.normalized();
            if (const Number length = mve::sqrt(length_sqr); length < min) {
                return result * min;
            }
            else if (length > max) {
                return result * max;
    ...
    [[nodiscard]] Vector2 operator*(Number scalar) const
    {
        return { x * scalar, y * scalar };
    }

    Vector2& operator*=(Number scalar)
    {
        x *= scalar;
        y *= scalar;
        return *this;
...
public:
    ...
    Vector2i& operator%=(const int val)
    {
        x %= val;
        y %= val;
        return *this;
    ...
    [[nodiscard]] Vector2i operator*(const int val) const
    {
        return { x * val, y * val };
    }
...
public:
    ...
    {
        return { mve::abs(x), mve::abs(y), mve::abs(z) };
    }

    [[nodiscard]] Vector3 ceil() const
    {
        return { mve::ceil(x), mve::ceil(y), mve::ceil(z) };
    }

    [[nodiscard]] Vector3 clamp(const Vector3 min, const Vector3 max) const
    {
        return { mve::clamp(x, min.x, max.x), mve::clamp(y, min.y, max.y), mve::clamp(z, min.z, max.z) };
    }
    ...
    [[nodiscard]] Vector3 normalized() const
    {
        ...
        if (const auto len = length(); len > 0) {
            const Number inverse_length = 1.0 / len;
            result = *this * inverse_length;
        }
        return result;
    }
    ...
    [[nodiscard]] Number distance_sqrd_to(const Vector3 to) const
    {
        ...
        return mve::sqrd(diff_x) + mve::sqrd(diff_y) + mve::sqrd(diff_z);
    }
    ...
    {
        return { mve::floor(x), mve::floor(y), mve::floor(z) };
    }

    [[nodiscard]] Number length_sqrd() const
    {
        return mve::sqrd(x) + mve::sqrd(y) + mve::sqrd(z);
    }
    ...
    {
        return { mve::lerp(x, to.x, weight), mve::lerp(y, to.y, weight), mve::lerp(z, to.z, weight) };
    }
    ...
    [[nodiscard]] bool is_equal_approx(const Vector3 other) const
    {
        return mve::is_equal_approx(x, other.x) && mve::is_equal_approx(y, other.y) && mve::is_equal_approx(z, other.z);
    }

    [[nodiscard]] bool is_zero_approx() const
    {
        return mve::is_zero_approx(x) && mve::is_zero_approx(y) && mve::is_zero_approx(z);
    }

    [[nodiscard]] Vector3 move_toward(const Vector3 to, const Number amount) const
    {
        if (this->distance_to(to) >= amount) {
            return to;
        }
    ...
    {
        return { mve::round(x), mve::round(y), mve::round(z) };
    }

    [[nodiscard]] Number angle(const Vector3 other) const
    {
        return mve::atan2(this->cross(other).length(), this->dot(other));
    }
    ...
    [[nodiscard]] Vector3 rotate(Vector3 axis, Number angle) const
    {
        ...
        angle /= 2.0f;
        Number a = mve::sin(angle);
        const Number b = axis.x * a;
        const Number c = axis.y * a;
        const Number d = axis.z * a;
        a = mve::cos(angle);
...
public:
    ...
    {
        return { mve::ceil(x), mve::ceil(y), mve::ceil(z), mve::ceil(w) };
    }

    [[nodiscard]] Vector4 clamp(const Vector4 min, const Vector4 max) const
    {
        return { mve::clamp(x, min.x, max.x),
                 mve::clamp(y, min.y, max.y),
                 mve::clamp(z, min.z, max.z),
                 mve::clamp(w, min.w, max.w) };
    }
    ...
    [[nodiscard]] Number distance_sqrd_to(const Vector4 to) const
    {
        ...
        return mve::sqrd(diff_x) + mve::sqrd(diff_y) + mve::sqrd(diff_z) + mve::sqrd(diff_w);
    }

    [[nodiscard]] Number distance_to(const Vector4 to) const
    {
        return mve::sqrt(distance_sqrd_to(to));
    }

    [[nodiscard]] Vector4 floor() const
    {
        return { mve::floor(x), mve::floor(y), mve::floor(z), mve::floor(w) };
    }

    [[nodiscard]] Number length_sqrd() const
    {
        return mve::sqrd(x) + mve::sqrd(y) + mve::sqrd(z) + mve::sqrd(w);
    }
    ...
    {
        return { mve::lerp(x, to.x, weight),
                 mve::lerp(y, to.y, weight),
                 mve::lerp(z, to.z, weight),
                 mve::lerp(w, to.w, weight) };
    }
    ...
    {
        return mve::is_equal_approx(x, other.x) && mve::is_equal_approx(y, other.y) && mve::is_equal_approx(z, other.z)
            && mve::is_equal_approx(w, other.w);
    }

    [[nodiscard]] bool is_zero_approx() const
    {
        return mve::is_zero_approx(x) && mve::is_zero_approx(y) && mve::is_zero_approx(z) && mve::is_zero_approx(w);
    }
...
public:
    ...
    [[nodiscard]] Number length() const
    {
        return mve::sqrt(static_cast<Number>(length_sqrd()));
    }
...
public:
    ...
    {
        return mve::is_equal_approx(x, other.x, epsilon) && mve::is_equal_approx(y, other.y, epsilon)
            && mve::is_equal_approx(z, other.z, epsilon) && mve::is_equal_approx(w, other.w, epsilon);
    }

    [[nodiscard]] bool is_zero_approx(Number epsilon) const
    {
        return mve::is_zero_approx(x, epsilon) && mve::is_zero_approx(y, epsilon) && mve::is_zero_approx(z, epsilon)
            && mve::is_zero_approx(w, epsilon);
    }
...
public:
    ...
    [[nodiscard]] static Matrix3 from_axis_angle(const Vector3<Number>& axis, const float angle)
    {
        ...
        const Vector3 axis_norm = axis.normalized();
    ...
    {
        float c = mve::cos(euler.x);
        float s = mve::sin(euler.x);
...
public:
    ...
    {
        return (*this)[0].is_equal_approx(other[0]) && (*this)[1].is_equal_approx(other[1])
            && (*this)[2].is_equal_approx(other[2]) && (*this)[3].is_equal_approx(other[3]);
    }

    [[nodiscard]] bool is_zero_approx() const
    {
        return (*this)[0].is_zero_approx() && (*this)[1].is_zero_approx() && (*this)[2].is_zero_approx()
            && (*this)[3].is_zero_approx();
    }
