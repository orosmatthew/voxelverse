...
inline Quaternion Quaternion::from_axis_angle(const Vector3& axis, const float angle)
{
    Vector4 data;

    if (const float axis_length = mve::length(axis); axis_length == 0) {
        data.x = 0.0f;
        data.y = 0.0f;
        data.z = 0.0f;
        data.w = 0.0f;
...
inline Quaternion Quaternion::spherical_linear_interpolate(const Quaternion& to, const float weight) const
{
...
inline Quaternion Quaternion::operator*(const int value) const
{
    return Quaternion(data * value);
}
inline void Quaternion::operator*=(const int value)
{
    data *= value;
}
inline Quaternion Quaternion::operator+(const Quaternion& other) const
...
inline const float& Quaternion::operator[](const int index) const
{
    return data[index];
}
...
{
    const float dot = mve::dot(from, to);
...
inline Quaternion spherical_linear_interpolate(const Quaternion& from, const Quaternion& to, const float weight)
{
